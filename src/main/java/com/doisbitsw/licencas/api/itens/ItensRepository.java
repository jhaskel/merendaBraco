...
public interface ItensRepository extends JpaRepository<Itens, Long> {
    ...
    List<Itens> findByPedidoAll(Long pedido);
    ...
    @Query(value = "SELECT sum(ite.total) as tot  FROM itens ite\n" +
            "INNER JOIN af ON af.code = ite.af\n" +
            "INNER JOIN categoria cat ON cat.id = ite.categoria\n" +
            "WHERE af.isativo= true AND cat.isalimento=true  and ite.ano = :ano AND ite.af > 0 ", nativeQuery = true)
    double findTotal(Long ano);

    @Query(value = "SELECT sum(ite.total) as tot  FROM itens ite\n" +
            "INNER JOIN af ON af.code = ite.af\n" +
            "INNER JOIN categoria cat ON cat.id = ite.categoria\n" +
            "WHERE af.isativo= true AND cat.isalimento=false  and ite.ano = :ano AND ite.af > 0 ", nativeQuery = true)
    double findTotalLimpeza(Long ano);
    ...
    @Query(value = "SELECT sum(ite.total) as tot  \n" +
            ...
            "INNER JOIN af ON af.code = ite.af\n" +
            "INNER JOIN categoria cat ON cat.id = ite.categoria\n" +
            "WHERE af.isativo= TRUE AND cat.isalimento=false  and ite.ano = :ano AND ite.af > 0\n" +
            "AND ite.isagro = FALSE and ite.nivel = :nivel", nativeQuery = true)
